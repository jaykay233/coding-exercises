 ...
 * 算法步骤
 * 1. 首先先获得sa和height数组
 * 2. 然后对sa[i]计算个数的前缀和得到结果
 * 3. 然后对sa[i]的前缀和数组二分查找位置满足k在对应的区间
 * 4. 得到所需要的字符串，但是还要判断位置在左侧的字符串是否也满足条件，所以需要判断条件
 * 5  第4步中判断的条件就是获取在sa数组中最靠右的二分位置，然后对满足条件的位置同样适用rmq找到最靠左侧满足条件的位置
 * 
 * ***/
